    ...
    <Transition name="fade">
      ...
      >
        <Transition name="scale">
          <div
            v-if="show"
            class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-white/20"
          >
            <!-- Icon -->
            ...
            <!-- Title -->
            <h3 v-if="title" class="text-xl font-bold text-slate-800 text-center mb-3">
              {{ title }}
            </h3>
            ...
            <div class="flex gap-3">
              <button
                @click="handleCancel"
                class="flex-1 bg-slate-100 text-slate-700 py-2.5 px-5 rounded-xl font-semibold hover:bg-slate-200 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98]"
              >
                {{ cancelText || 'Cancel' }}
              </button>
              <button
